...
try {
	...
	const configExt = [
		"ts",
		"json",
	].find(ext => fs.existsSync(path.join(basePath, `${workflowName}.${ext}`)));
	if (configExt) {
	...
	if (config.scriptsHandling !== "retain-all") {
		...
		if (config.scriptsHandling === "remove-all") {
			...
			pkgJson.scripts = pkgJson.scripts?.start
				? {
						start: pkgJson.scripts.start,
					}
				: {};
		} else if (config.scriptsHandling === "keep-build") {
			pkgJson.scripts = pkgJson.scripts?.build
				? {
						start: pkgJson.scripts.build,
					}
				: {};
		} else if (
			config.scriptsHandling === "custom-list" &&
	...
	const typedocs =
		config.typeDocs === 2 || (config.typeDocs === 1 && config.publishToNpm);
